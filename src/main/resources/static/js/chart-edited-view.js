window.addEventListener('load', function() {
    const userData = document.querySelector('#data-from-user');
    ...
    // code to be executed when the window is loaded
        ...
        var chart = new CanvasJS.Chart("chart-container", finalTrip);
        chart.options.data = visitorsData["Your trip"];
        chart.render();

        function visitorsChartDrilldownHandler(e) {
            chart = new CanvasJS.Chart("chart-container", visitorsDrilldownedChartOptions);
            chart.options.data = visitorsData[e.dataPoint.name];
            chart.options.title = { text: e.dataPoint.name }
            chart.render();
            $("#back-Button").toggleClass("invisible");
        }

        $("#back-Button").click(function() {
            $(this).toggleClass("invisible");
            chart = new CanvasJS.Chart("chart-container", finalTrip);
            chart.options.data = visitorsData["Your trip"];
            chart.render();
        });
