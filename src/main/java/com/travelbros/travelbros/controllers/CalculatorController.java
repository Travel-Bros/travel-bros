...
//        calculatorDao.save(calculator);
        ...
        User user = userDao.findById(Utils.currentUserId());
        Vehicle vehicle = vehicleDao.findById(trip.getVehicleId().getId());
        Budget budget = trip.getTripBudget();
        double tripStops = Calculator.numberOfStops(trip.getDistance(), user.getUserVehicles(trip.getVehicleId()))
